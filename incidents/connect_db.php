    ...
    function sender($token,$url,$execute,$select,$from,$where){
        ...
        //print_r($trial);
        //return array with 3 keys, status for query success/fail, message for fetch_data's raw data or error message when status=false.

        //print_r(json_decode($trial['message'],TRUE));
        //for fetch_data to decode message

        return $trial;
    }
// sender('Dasebase-#6675','172.21.146.197/db/database_mgr.php','insert_data',Null,'incident_report','first_name=hi,last_name=bye,
//   phone_number=333333,postal_code=333333,address=herenotthere
//   ,assistance=Gas,description=extra,
//   create_date_time=now,status=Resolved,aid=1');
//      sender('Dasebase-#6675','172.21.146.197/db/database_mgr.php','fetch_data','*','incident_report',Null);
